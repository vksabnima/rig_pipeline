...
sub-components. Multi-model benchmarking across four LLMs (GPT-4o, Claude~Sonnet,
Qwen3-235B, Llama-3-8B) against three independent human-validated
gold sets yields three findings: (i) inter-model RIF ordering is
\emph{document-dependent} --- Claude~Sonnet significantly exceeds
GPT-4o on Basel~III (non-overlapping 95\% bootstrap CIs, gap 0.052)
but all four models are statistically tied on Reg~BI and GDPR under
independent gold; (ii) among frontier-class extractors of comparable
output entropy, above-threshold cross-document alignment precision
is near-invariant (CAS range 0.003), making similarity-based
alignment robust to frontier-extractor choice; lower-entropy
extractors inflate CAS through template-driven embedding-space
clustering; (iii) the smaller dense open model jointly maximises
three output-entropy-sensitive metrics (OAL 0.820, CAS 0.360,
HRR-Stability 0.413) while landing mid-pack on gold-aligned RIF,
revealing \emph{template-uniformity inflation} --- a failure mode of
composite extraction metrics in which uniform, high-coverage JSON
outputs inflate every unsupervised quality signal without
corresponding fidelity gains. Error analysis of the 30 lowest-fidelity GPT-4o
extractions shows cross-reference dependencies drive 90\% of
failures, identifying a tractable architectural target for future
work.
\end{abstract}
...
Llama-3-8B establishes three principal findings: (i) inter-model
fidelity ordering is \emph{document-dependent} rather than universal
---Claude~Sonnet significantly exceeds GPT-4o on rule-dense Basel~III
but all four models are statistically tied on conduct-based Reg~BI
and on structurally-clean GDPR under independent human-validated
gold; (ii) among frontier-class models of comparable output entropy,
...
lower-entropy extractors produce tighter embedding-space clustering
and therefore inflate CAS; and (iii) the smaller dense open model
(Llama-3-8B) exhibits \textit{template-uniformity inflation}---
jointly maximising three output-entropy-sensitive metrics (OAL, CAS,
HRR-Stability) while landing mid-pack on gold-aligned RIF---
demonstrating that unsupervised composite extraction metrics can be
driven by low output entropy rather than by genuine extraction
quality unless cross-checked against independent gold-validated RIF.
...
$^\ast$RIF is computed against three independent human-validated
gold sets produced specifically for this study: Basel~III (758
rows, extended with a 300-clause stratified protocol-controlled
human pass, yielding pooled presence~$\kappa = 0.606$ and
content-alignment 0.713 over 2{,}400 field judgments); Reg~BI
(100-clause independent human gold, eliminating the clause-
identifier collision artefact from earlier iterations); GDPR
(50-clause independent human gold, Claude~Sonnet excluded from the
GDPR~RIF row to avoid self-evaluation circularity --- see
footnote~$\flat$).
...
\multirow{3}{*}{\shortstack{Claude\\Sonnet}}
  & Basel~III  & 0.971 & 0.720 & \textbf{0.616} [0.600, 0.632] & 0.323 & 0.433 & 1.000 & 0.175 \\
  & Reg~BI     & 0.899 & 0.753 & 0.486 [0.428, 0.537] & 0.323 & 0.433 & 1.000 & 0.175 \\
...
\multirow{3}{*}{GPT-4o}
  ...
  & Reg~BI     & 0.818 & 0.642 & 0.457 [0.404, 0.513] & 0.326 & 0.392 & 1.000 & 0.203 \\
  & GDPR       & 0.948 & 0.735 & \textbf{0.533} [0.470, 0.593] & \multicolumn{1}{c}{$0^{\S}$} & 0.392 & 1.000 & 0.203 \\
\midrule
\multirow{3}{*}{\shortstack{Qwen3-235B$^{\P}$\\(open)}}
  & Basel~III  & \textbf{1.000} & 0.753 & 0.543 [0.531, 0.556] & 0.323 & 0.461 & 1.000 & 0.302 \\
  & Reg~BI     & 0.920 & 0.771 & 0.489 [0.431, 0.544] & 0.323 & 0.461 & 1.000 & 0.302 \\
  & GDPR       & 0.998 & \textbf{0.803} & 0.519 [0.454, 0.578] & 0.331 & 0.461 & 1.000 & 0.302 \\
\midrule
\multirow{3}{*}{\shortstack{Llama-3-8B\\(open, 8B)}}
  & Basel~III  & 1.000 & \textbf{0.863} & 0.549 [0.538, 0.561] & \textbf{0.360} & 0.514 & 1.000 & 0.413 \\
  & Reg~BI     & 0.997 & 0.776 & 0.503 [0.449, 0.554] & \textbf{0.360} & 0.514 & 1.000 & 0.413 \\
  & GDPR       & 1.000 & 0.843 & 0.502 [0.447, 0.560] & 0.345 & 0.514 & 1.000 & 0.413 \\
\bottomrule
\end{tabular}
...
\node[font=\scriptsize,text=white] at (2.5*\cw,0.5*\ch){0.526};
...
\node[font=\scriptsize,text=white] at (2.5*\cw,1.5*\ch){0.510};
...
\node[font=\scriptsize,text=white] at (2.5*\cw,2.5*\ch){0.516};
...
\node[font=\scriptsize,text=white] at (2.5*\cw,3.5*\ch){0.551};
...
re-rendered against the three independent human-validated gold sets
used throughout this study. All four LLMs (two closed frontier and
two open-weights) are reported.
$^{\flat}$Claude~Sonnet GDPR RIF is a self-evaluation against a
Claude-authored gold set (see Table~\ref{tab:fullresults}); shown
...
\toprule
 & \multicolumn{3}{c|}{Baseline (regex)} &
   ...
   \multicolumn{3}{c|}{Qwen3-235B} &
   \multicolumn{3}{c}{Llama-3-8B} \\
Doc & RCC & OAL & RIF & RCC & OAL & RIF & RCC & OAL & RIF & RCC & OAL & RIF & RCC & OAL & RIF \\
\midrule
Basel~III & 0.974 & 0.249 & 0.405 & 0.996 & 0.689 & 0.564 & 0.971 & 0.720 & \textbf{0.616} & 1.000 & 0.753 & 0.543 & 1.000 & 0.863 & 0.549 \\
Reg~BI    & 0.912 & 0.253 & 0.316 & 0.818 & 0.642 & 0.457 & 0.899 & 0.753 & 0.486 & 0.920 & 0.771 & 0.489 & 0.997 & 0.776 & \textbf{0.503} \\
GDPR      & 0.982 & 0.460 & 0.424 & 0.948 & 0.735 & \textbf{0.533} & 0.994 & 0.771 & 0.538$^{\flat}$ & 0.998 & 0.803 & 0.519 & 1.000 & 0.843 & 0.502 \\
\bottomrule
\end{tabular}
\label{tab:baseline}
\end{table}
...
\noindent
The pattern is consistent across all three documents and all four
...
is large and consistent ($+0.27$ to $+0.61$ absolute across docs and
all four LLMs): LLMs reliably populate three or more fields per
clause whereas the baseline averages closer to two. The RIF gap
over the regex baseline is $+0.17$ to $+0.21$ for Claude~Sonnet,
$+0.11$ to $+0.16$ for GPT-4o, $+0.10$ to $+0.17$ for Qwen3-235B,
and $+0.08$ to $+0.19$ for Llama-3-8B (excluding the self-evaluation
Claude~Sonnet GDPR cell), confirming that LLM-extracted field values
are substantially more faithful to gold content than regex-captured
spans even when the regex correctly identifies that a field exists. Taken together,
...
and an independent protocol-controlled human annotation pass on a
400-clause stratified random sample of Basel~III (pilot 100 clauses
plus an expanded 300-clause follow-up using a revised annotation
protocol with worked examples and field-decision rules).
``Presence $\kappa$'' is Cohen's $\kappa$ on the binary field-present
judgment per row; ``Content alignment'' is the fraction of rows where
both raters marked the field present \emph{and} their string values
satisfy bidirectional substring containment after lower-casing and
whitespace stripping. Annotators were not calibrated directly against
each other; the human pass was performed independently without prior
exposure to the auto-annotation.}
\centering
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
subject     & 400 & 0.131 & 213 & 0.826 \\
obligation  & 400 & 0.000 & 369 & 0.650 \\
condition   & 400 & 0.271 & 192 & 0.708 \\
threshold   & 400 & 0.351 & 32  & 0.656 \\
deadline    & 400 & 0.289 & 14  & 0.786 \\
exception   & 400 & 0.484 & 7   & 0.857 \\
\midrule
Overall (2{,}400 judgments) & 2{,}400 & \textbf{0.606} & 827 & \textbf{0.713} \\
\bottomrule
\end{tabular}
\label{tab:kappa}
\end{table}
...
\noindent
With the protocol-controlled expansion to 400 clauses, pooled
presence~$\kappa$ rises from the pilot value of 0.574 to
\textbf{0.606} (moderate-to-substantial on Landis and Koch), and
content alignment rises substantially from 0.443 to \textbf{0.713}
--- a consequence of the revised annotation protocol's explicit
field-decision rules and worked examples, which nearly halved
annotator content-drift relative to the pilot.
The per-field $\kappa$ of 0.000 on the \textit{obligation} row
reflects a known chance-correction degeneracy: when both raters
mark a field present in over 90\% of cases (here both annotators
marked \textit{obligation} present on $> 92\%$ of clauses), the
expected-agreement term in Cohen's $\kappa$ approaches the observed-
agreement term, driving chance-corrected $\kappa$ toward zero even
at very high raw agreement (0.923 raw agreement on \textit{obligation},
0.65--0.73 content alignment). For the highest-base-rate fields the
content-alignment column is the informative metric; for the
lower-base-rate fields both $\kappa$ and content alignment are
informative and both improved substantially on the protocol-controlled
expansion (Table~\ref{tab:protocol_effect}).
Table~\ref{tab:protocol_effect} isolates the protocol effect by
computing per-field $\kappa$ and content alignment separately on
the 100-clause pilot (ad-hoc protocol) and the 300-clause
protocol-controlled expansion: on every field the content-alignment
score rose sharply, with subject and condition rising from
0.54~/~0.39 to 1.00~/~0.91 respectively, and the low-frequency
deadline and exception fields rising from near-zero alignment
(small-$n$ instability) to 1.00 with the larger both-present samples.

\begin{table}[!htbp]
\caption{Effect of the revised annotation protocol on per-field
inter-annotator agreement. Pilot used an ad-hoc annotation
protocol on 100 clauses; the protocol-controlled expansion used
explicit field-decision rules and worked examples
(\texttt{ANNOTATION\_GUIDE.md}) on an additional 300 clauses.
Content alignment rose sharply on every field, indicating that
definitional drift (not task difficulty) was the dominant source
of disagreement in the pilot.}
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{3}{c}{Pilot (100 clauses)} & \multicolumn{3}{c}{Protocol (300 clauses)} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
Field & $\kappa$ & $n_{\text{b.p.}}$ & C.A. & $\kappa$ & $n_{\text{b.p.}}$ & C.A. \\
\midrule
subject     & 0.144  & 81 & 0.543 & 0.128 & 132 & \textbf{1.000} \\
obligation  & 0.000  & 87 & 0.402 & 0.000 & 282 & \textbf{0.727} \\
condition   & 0.251  & 74 & 0.392 & 0.265 & 118 & \textbf{0.907} \\
threshold   & 0.248  & 16 & 0.500 & 0.394 & 16  & \textbf{0.812} \\
deadline    & $-$0.011 & 3  & 0.000 & 0.550 & 11  & \textbf{1.000} \\
exception   & 0.185  & 1  & 0.000 & 0.656 & 6   & \textbf{1.000} \\
\midrule
Pooled      & 0.587  & 262 & 0.443 & 0.614 & 565 & \textbf{0.839} \\
\bottomrule
\end{tabular}
\label{tab:protocol_effect}
\end{table} Per-field $\kappa$
remains depressed on high-frequency fields (subject, obligation)
because both raters mark these present in over 90\% of clauses,
leaving little room for chance-corrected agreement to register; the
content-alignment column is the more informative view here and shows
consistently high agreement (0.65--0.86) across all six fields. On
the low-frequency fields (deadline, exception) the revised protocol
increased the both-present sample size (from $n=3$ to 14 for
deadline; $n=1$ to 7 for exception) and brought content alignment
up sharply (to 0.786 and 0.857 respectively).
We retain the human-validated Basel~III gold standard as the
working reference for Basel~III RIF in Table~\ref{tab:fullresults}
but flag the inter-annotator noise as a material caveat: absolute
RIF values should be read with $\sigma \sim 0.05$--$0.10$
uncertainty arising from annotation disagreement. The one
significant inter-model RIF \emph{difference} we report --- the
Basel~III Claude--GPT-4o gap of 0.052 with non-overlapping bootstrap
CIs --- remains valid under this noise level because all four models
are scored against identical reference content; the absence of
significant inter-model gaps on Reg~BI and GDPR under independent
gold (Section~\ref{sec:discussion}) is likewise robust to the same
noise.

\subsection{Frontier Model Fidelity and Statistical Significance}
\label{sec:discussion}

Under the expanded independent human gold, the inter-model RIF
picture is \emph{document-dependent}, not universal.

On \textbf{Basel~III} (rule-dense, quantitative) Claude~Sonnet
achieves statistically significantly higher RIF than GPT-4o
(0.616~[0.600,~0.632] vs.\ 0.564~[0.551,~0.576], gap~$=0.052$,
non-overlapping CIs). Qwen3-235B (0.543 [0.531, 0.556]) and
Llama-3-8B (0.549 [0.538, 0.561]) both sit below GPT-4o at the
point estimate but overlap GPT-4o's CI (a 0.005-wide overlap at
the top of each open-model CI), so Claude~Sonnet is the only model
that is statistically ahead of the remaining three; the
GPT-4o~/~Qwen3~/~Llama-3-8B cluster is within a 0.021 point-estimate
range with pairwise overlapping CIs.

On \textbf{Reg~BI} (100-clause independent human gold,
conduct-based) all four models are \emph{statistically tied}:
Llama-3-8B~0.503 [0.449, 0.554], Qwen3-235B~0.489 [0.431, 0.544],
Claude~Sonnet~0.486 [0.428, 0.537], GPT-4o~0.457 [0.404, 0.513].
All six pairwise 95\% CIs overlap.

On \textbf{GDPR} (50-clause independent human gold, Claude excluded
for self-evaluation circularity) GPT-4o~(0.533 [0.470, 0.593]),
Qwen3-235B~(0.519 [0.454, 0.578]) and Llama-3-8B~(0.502 [0.447, 0.560])
are again statistically tied.

The headline implication is that the Claude--GPT-4o fidelity gap we
previously reported on \textit{both} documents was a genuine finding
on Basel~III but an artefact of Basel~III gold-content contamination
on Reg~BI: once Reg~BI is scored against its own independent gold,
the inter-model differences disappear. Inter-model extraction
fidelity is therefore document-dependent --- rule-dense, specificity-
sensitive text (Basel~III) surfaces a measurable gap between frontier
closed models, while conduct-based text (Reg~BI) and structurally-
clean text (GDPR) do not distinguish them under the metrics we use.
Figure~\ref{fig:rif_by_doc} visualises this pattern: Claude~Sonnet's
Basel~III error bar sits cleanly above the other three
(non-overlapping), whereas on Reg~BI and GDPR all model error bars
...
\begin{tikzpicture}
\begin{axis}[
  ybar, bar width=0.24cm,
  width=0.92\textwidth, height=6.5cm,
  xlabel={Document (independent human-validated gold)},
  ylabel={RIF (95\% bootstrap CI)},
  ymin=0.35, ymax=0.70,
  symbolic x coords={Basel III, Reg BI, GDPR},
  xtick=data,
  ...
  error bars/error mark options={rotate=90, mark size=1.7pt, line width=0.7pt},
]
\addplot+[fill=navyblue!80, draw=navyblue, error bars/.cd, y dir=both, y explicit]
  coordinates {
    (Basel III, 0.564) +- (0, 0.012)
    (Reg BI,    0.457) +- (0, 0.053)
    (GDPR,      0.533) +- (0, 0.061)
  };
\addlegendentry{GPT-4o}
\addplot+[fill=burntorange!80, draw=burntorange, error bars/.cd, y dir=both, y explicit]
  coordinates {
    (Basel III, 0.616) +- (0, 0.016)
    (Reg BI,    0.486) +- (0, 0.054)
  };
\addlegendentry{Claude Sonnet$^{\flat}$}
\addplot+[fill=steelblue!70, draw=steelblue, error bars/.cd, y dir=both, y explicit]
  coordinates {
    (Basel III, 0.543) +- (0, 0.012)
    (Reg BI,    0.489) +- (0, 0.056)
    (GDPR,      0.519) +- (0, 0.062)
  };
\addlegendentry{Qwen3-235B}
\addplot+[fill=forestgreen!70, draw=forestgreen!80!black, error bars/.cd, y dir=both, y explicit]
  coordinates {
    (Basel III, 0.549) +- (0, 0.011)
    (Reg BI,    0.503) +- (0, 0.053)
    (GDPR,      0.502) +- (0, 0.057)
  };
\addlegendentry{Llama-3-8B}
\end{axis}
\end{tikzpicture}
\caption{\textbf{Document-dependent RIF ordering.} Per-model RIF
with 95\% bootstrap confidence-interval error bars, computed against
three independent human-validated gold sets (Basel~III $n=758$,
Reg~BI $n=100$, GDPR $n=50$). Claude~Sonnet's Basel~III bar is the
only bar without CI overlap with any other model; on Reg~BI all four
bars overlap pairwise, and on GDPR the three compared models
(Claude~Sonnet excluded for self-evaluation circularity) also
overlap pairwise. GDPR values for Claude~Sonnet are reported as a
self-consistency measurement only ($0.538$, not directly comparable)
and are omitted from this figure.}
\label{fig:rif_by_doc}
\end{figure}
...
Llama-3-8B Exhibits Template-Uniformity}
\label{sec:openweight}

The four-model comparison separates two distinct behavioural
profiles in open-weights extraction.
...
(0.753~/~0.771~/~0.803), and RIF against independent human gold
(0.543~/~0.489~/~0.519). On Basel~III Qwen3 ties Llama-3-8B for the
highest RCC; on GDPR it sets the highest OAL among all four models.
...
strikingly different behavioural profile driven by low output
entropy rather than by a fidelity deficit. RCC is essentially
perfect (0.998 average), OAL is the highest of any model
benchmarked (0.820 average versus 0.737 for the next-highest
Claude~Sonnet), and Reg~BI RIF is in fact the highest point
estimate (0.503) among the four models, though statistically tied
with the other three under the 100-clause independent Reg~BI gold
(Table~\ref{tab:fullresults}). On Basel~III Llama-3-8B sits in the
GPT-4o / Qwen3 / Llama-3-8B statistical cluster (0.549 [0.538,
0.561]) and is significantly below Claude~Sonnet (gap 0.067,
non-overlapping CIs). On GDPR it is the lowest of the three
compared point estimates (0.502) but within overlapping CIs of
GPT-4o (0.533) and Qwen3 (0.519).

The interesting observation is not that Llama-3-8B has inflated
unsupervised metrics \emph{without} supporting fidelity --- fidelity
is competitive. Rather, Llama-3-8B jointly maximises \emph{three
output-entropy-sensitive metrics} (OAL 0.820, CAS 0.360,
HRR-Stability 0.413) while sitting mid-pack or lower on RIF. We
interpret this pattern as \emph{template-uniformity inflation}: the
model produces highly uniform, high-coverage JSON outputs whose
field values (a) always fill most tuple positions (inflating OAL),
(b) cluster tightly in sentence-embedding space because they share
lexical templates (inflating CAS through higher above-threshold
pair density --- 6{,}253 Basel~III$\leftrightarrow$Reg~BI pairs
versus $\approx 1{,}600$ for the frontier models), and (c) change
less under adversarial perturbation because the output is more
strongly anchored to the model's template prior than to the input
text (inflating HRR-Stability). The HRR-Stability value of 0.413 is
illustrative: roughly 2.4$\times$ Claude~Sonnet's (0.175) and
2.0$\times$ GPT-4o's (0.203) on 20-clause adversarial samples drawn
under the same random seed and five perturbation types, but this
stability is template-anchored, not fidelity-anchored.
The pilot experiment with an under-configured Llama~3.2 (3B, local
CPU inference, extraction sampling) is reported in
Appendix~\ref{app:llama32} and shows the opposite regime --- sparse
extraction also inflates HRR-Stability, but via having too little
content to perturb rather than via template uniformity.

The practical implication is that high values of OAL, CAS, and
HRR-Stability do not, on their own, certify extraction quality
above frontier extractors. They can arise from genuine thoroughness
(Basel~III Qwen3 OAL 0.753, RIF 0.543) or from lower output
entropy (Basel~III Llama-3-8B OAL 0.863, RIF 0.549). We recommend
reporting RCC, OAL, and RIF jointly, and decomposing HRR into
HRR-Detection and HRR-Stability, whenever composite scores are
...
One non-trivial implementation artifact that affects interpretation
is documented here and in \S\ref{sec:limitations}:
\textit{(i)}~earlier iterations of this manuscript reported Reg~BI
RIF against Basel~III gold content (a clause-identifier collision
artefact); the current manuscript reports Reg~BI RIF against a
fresh 100-clause independent human gold, and the revised numbers
appear in Table~\ref{tab:fullresults}. Additionally,
...
Claude~Sonnet--GPT-4o RIF gap on Basel~III, frontier-invariant
above-threshold CAS precision among entropy-matched extractors,
and Llama-3-8B template-uniformity inflation in OAL, CAS, and
HRR-Stability).
...
\textbf{Output-entropy-driven stability inflation.}
HRR-Stability varies by a factor of 2--3$\times$ across the four
models (Claude~Sonnet 0.175, GPT-4o 0.203, Qwen3-235B 0.302,
Llama-3-8B 0.413) and this variation does not track extraction
richness: Llama-3-8B has simultaneously the highest OAL (0.820) and
the highest HRR-Stability (0.413). The consistent explanation
across our three regimes --- frontier closed models (high entropy,
moderate stability), capable open MoE (Qwen3-235B, intermediate),
dense small open (Llama-3-8B, low entropy, high stability), and the
sparsity-collapsed pilot (Llama~3.2 in Appendix~\ref{app:llama32},
near-zero output, also high stability) --- is that HRR-Stability is
dominated by output-template entropy rather than by the
richness--robustness tradeoff that composite-HRR framings
implicitly assume. Systems optimised on composite HRR without
RIF cross-checking will therefore prefer models whose outputs vary
little with input (template-uniform or sparsity-collapsed) over
models with richer, input-responsive outputs, even when the former
have lower gold-aligned fidelity. We recommend that RegTech
deployments decompose HRR into HRR-Detection (which is
uninformative at 1.000 across all four models in our experiments)
and HRR-Stability (interpretable only alongside RIF), and select
models on RIF rather than composite HRR.
...
All three independent human-validated gold sets (Basel~III: 400
clauses pooled across pilot + protocol-controlled expansion;
Reg~BI: 100 clauses independently annotated for this revision;
GDPR: 50 clauses independently annotated) are released with the code.
Claude~Sonnet is excluded from the GDPR RIF comparison
because the 50-clause GDPR gold set was authored by Claude~Sonnet
in an earlier pilot and scoring Claude~Sonnet against Claude-authored
gold is self-evaluative; a fully independent GDPR gold pass of the
same 50 clauses (and an expansion to all 498) remain future work.
The Basel~III gold standard used for the main RIF results in
Table~\ref{tab:fullresults} is Claude~Sonnet-pre-annotated and
Claude~Sonnet-validated, then corrected and extended by a
400-clause human pass; consequently Claude~Sonnet's Basel~III
RIF advantage (gap 0.052 over GPT-4o, the paper's one
statistically significant inter-model gap) is partly subject to a
home-field effect that the human IAA work mitigates but does not
fully eliminate. The 0.713 content-alignment figure on the 400
human-reviewed clauses bounds this effect (the gold content
reflects $\geq 0.713$ human agreement with Claude's original
annotations across both-present fields); a fully
human-originated Basel~III gold set would be the cleanest future
validation, and we note the remaining possibility that the gap
is partially Claude-gold-aligned rather than fidelity-driven.
Inter-annotator $\kappa$ on the Basel~III sample is moderate-to-
substantial (pooled 0.606; content alignment 0.713) but per-field
$\kappa$ on the two highest-base-rate fields (subject, obligation)
remains depressed by base-rate compression and is best interpreted
through the content-alignment column.
Adversarial routines use 20 clauses per model rather than the full
corpus; per-perturbation change rates are reported for the two
closed frontier models only in Figure~\ref{fig:adversarial}.
...
practical implications: inter-model fidelity ordering is
document-dependent (Claude~$>$~GPT-4o on rule-dense Basel~III but not
on Reg~BI or GDPR under independent human-validated gold);
above-threshold cross-document alignment precision is near-invariant
among frontier-class extractors; and composite extraction metrics
can systematically overstate smaller-model quality via
template-uniformity inflation unless cross-checked against
independent gold-validated RIF.
...
sparsity extreme; the main-text \emph{template-uniformity inflation}
