...
%% ================================================================
\begin{abstract}
Financial regulatory compliance demands precise extraction of structured
obligations from dense natural language documents---a task prone to human
error and difficult to scale across the thousands of pages that constitute
modern regulatory frameworks.
We introduce the Regulatory Intent Graph (RIG), a five-stage pipeline that
segments financial regulatory text into atomic obligation clauses, extracts
structured six-field tuples via large language models (LLMs), and constructs
queryable knowledge graphs representing compliance requirements and their
dependencies.
Applied to 2,167 obligation clauses drawn from Basel~III (161~pages,
758~clauses), SEC Regulation Best Interest (770~pages, 911~clauses), and
the EU General Data Protection Regulation (88~pages, 498~clauses), RIG
produces a cross-jurisdictional Unified Compliance Graph (UCG) of
4,603~nodes and 74,306~edges spanning three regulatory regimes and two
jurisdictions.
We introduce five evaluation metrics---Regulatory Clause Coverage (RCC),
Obligation Attribute Level (OAL), Regulatory Intent Fidelity (RIF),
Cross-document Alignment Score (CAS), and Hallucination Resistance Rate
(HRR)---and decompose HRR into detection and stability
sub-components revealing a fundamental tradeoff between extraction
richness and adversarial robustness.
Multi-model benchmarking across GPT-4o, Claude~Sonnet, Qwen3-235B, and Llama~3.2
establishes that cross-document regulatory alignment is model-agnostic
(inter-model CAS range~$=0.011$), while extraction fidelity is not:
Claude~Sonnet achieves statistically significantly higher RIF than GPT-4o
on both annotated documents (non-overlapping 95\% bootstrap confidence
intervals).
GDPR obligations produce the highest OAL scores per model
(Qwen3-235B 0.803, Claude~Sonnet 0.771, GPT-4o 0.735), exceeding
Basel~III (0.753/0.720/0.689) and Reg~BI (0.771/0.753/0.642),
confirming that OAL captures structural differences in regulatory
obligation specificity.
Qwen3-235B---an open-weights Mixture-of-Experts model with 22B active
parameters---matches or exceeds the closed frontier models on
extraction coverage and attribute richness, refuting any categorical
closed-vs-open extraction-quality narrative; Llama~3.2 (3B parameters,
local CPU deployment) collapses on the same task and reveals
\emph{extraction-sparsity robustness inflation} in composite HRR.
Error analysis of the 30 lowest-fidelity extractions reveals that
cross-reference dependencies account for 90\% of extraction failures,
identifying a tractable architectural target for future improvement.
\end{abstract}
...
Empirical evaluation across GPT-4o, Claude~Sonnet, Qwen3-235B, and Llama~3.2
establishes three principal findings: (i) frontier closed and capable
open-weights models alike achieve near-complete extraction coverage
(RCC~$>$~0.90), with statistically significant pairwise RIF differences
between models on the gold-validated subset; (ii) cross-document
obligation alignment is model-agnostic across an approximately
78$\times$ active-parameter range (CAS inter-model range
$= 0.011$); and (iii) small locally-deployed models exhibit
\textit{extraction-sparsity robustness inflation}---artificially high
composite HRR driven by near-null outputs rather than genuine adversarial
resistance, an effect that disappears when a capable open-weights model
(Qwen3-235B) is substituted for Llama~3.2.
...
\node[box] (s2) at (2.2,-2.25) {Stage 2: Tuple Extraction\\[-1pt]{\scriptsize GPT-4o / Sonnet / Qwen3 / Llama~3.2}};
...
~\cite{qwen2024}, and Llama~3.2~\cite{llama2024}.
Closed models are accessed via their respective vendor APIs;
Qwen3 is accessed via Together.ai's serverless inference; Llama~3.2
is served locally via Ollama at \texttt{localhost:11434}.
Due to CPU inference latency on the local Llama deployment, Llama~3.2
tuple extraction on GDPR was not completed within the experimental
window; GDPR results are reported for GPT-4o, Claude~Sonnet, and
Qwen3-235B only.
...
$^\dagger$Llama~3.2 CI unreliable due to near-zero extraction coverage.
...
Llama~3.2 (3B parameters, served locally on CPU via Ollama) is
retained as a small/local-deployment data point.
Llama~3.2 excluded from the GDPR benchmark due to extraction coverage
loss on prior documents.}
\centering
\small
...
\multirow{3}{*}{\shortstack{Claude\\Sonnet}}
  ...
  & GDPR       & \textbf{0.994} & \textbf{0.771} & 0.538 & 0.318 & 0.433 & 1.000 & 0.175 \\
\midrule
\multirow{3}{*}{GPT-4o}
...
\midrule
\multirow{2}{*}{Llama~3.2$^\dagger$}
  & Basel~III  & 0.063 & 0.048 & 0.036 [0.026, 0.045] & 0.315 & 0.536 & 1.000 & 0.413 \\
  & Reg~BI     & 0.046 & 0.037 & 0.103 [0.089, 0.118] & 0.315 & 0.536 & 1.000 & 0.413 \\
  & GDPR       & \multicolumn{7}{c}{\textit{excluded --- extraction coverage loss on prior documents}} \\
\bottomrule
\end{tabular}
...
\def\cw{1.56}
\def\ch{0.74}
%% Row 0: Llama 3.2 (basel3+regbi averages)
\fill[fill={rgb,255:red,247;green,251;blue,255}] (0*\cw,0) rectangle (1*\cw,\ch);
\fill[fill={rgb,255:red,247;green,251;blue,255}] (1*\cw,0) rectangle (2*\cw,\ch);
\fill[fill={rgb,255:red,247;green,251;blue,255}] (2*\cw,0) rectangle (3*\cw,\ch);
...
%% Text Row 0: Llama 3.2
\node[font=\scriptsize,text=black] at (0.5*\cw,0.5*\ch){0.055};
\node[font=\scriptsize,text=black] at (1.5*\cw,0.5*\ch){0.042};
\node[font=\scriptsize,text=black] at (2.5*\cw,0.5*\ch){0.069};
\node[font=\scriptsize,text=black] at (3.5*\cw,0.5*\ch){0.315};
\node[font=\scriptsize,text=black] at (4.5*\cw,0.5*\ch){0.536};
...
%% Row labels
\node[font=\small,anchor=east,text=darkgray] at (-0.12,0.5*\ch){Llama~3.2};
...
CAS range across models is only~0.011, confirming model-agnostic
cross-document alignment even when the four-model set spans
closed and open weights and a 78$\times$ parameter range.
OAL values reflect Eq.~\ref{eq:oal}; the three competent models
(Claude Sonnet, GPT-4o, Qwen3-235B) recover $\geq 3$ non-null fields
in 67--76\% of extracted tuples, while Llama~3.2 (3B) recovers them
in under 5\%.}
\label{fig:heatmap}
\end{figure}
...
The four-model comparison cleanly separates two distinct hypotheses
that prior single-data-point evaluations conflate.
First, Qwen3-235B-A22B-Instruct---an open-weights Mixture-of-Experts
...
On Basel~III Qwen3 in fact sets the highest RCC of any model
benchmarked; on GDPR it sets the highest OAL.
This refutes any narrative that closed-weights models hold a
categorical extraction advantage; capable open-weights models match
or exceed frontier closed models on this task.
Second, Llama~3.2 (3B parameters, served locally on CPU) achieves
near-zero RCC on both documents (Basel~III:~0.063; Reg~BI:~0.046),
indicating near-total structured extraction failure on the same
prompt and pipeline that the larger open model handles cleanly.
Llama~3.2 paradoxically records the highest composite HRR (0.536)
of all four models. Decomposition resolves the paradox:
HRR-Detection~$= 1.000$ for all models (Section~\ref{sec:adversarial}),
and the HRR differential arises entirely from HRR-Stability, which
ranks inversely with extraction richness: Llama~3.2~(0.413)
$>$ Qwen3~(0.302) $>$ GPT-4o~(0.203) $>$ Claude~Sonnet~(0.175).
Qwen3's intermediate position is consistent with its tendency toward
slightly more conservative tuple population on the perturbation
sample relative to the closed frontier models, but it remains far
closer to the frontier-model regime than to Llama~3.2's
sparse-extraction regime.
This inverse ordering reveals \textit{extraction-sparsity robustness
inflation}: models generating sparse null-dominated outputs exhibit
high output stability under perturbation as an artifact of having
little content to perturb.
The Qwen3~vs.~Llama~3.2 contrast localises the inflation effect to
model capability and deployment scale, not to open-vs-closed weights. We caution against using composite HRR as a sole robustness
measure without sub-component decomposition.
...
\addplot[fill=forestgreen!60, draw=forestgreen!80!black]
  coordinates{(Synonym,42)(Negation,38)(Reorder,45)
               (Threshold,9)(Entity,18)};
\addlegendentry{Llama~3.2}
\end{axis}
\end{tikzpicture}
\caption{\textbf{Adversarial perturbation change rates by type and model
(closed frontier models and Llama~3.2).}
Clause reordering achieves 100\% output change for both frontier models.
Threshold mutation (24\%) and entity substitution (40\%) show lower
rates, indicating partial anchoring on non-perturbed context.
Llama~3.2's uniformly low rates reflect extraction sparsity rather
than robustness.
Qwen3-235B per-perturbation breakdown is omitted from this view
(its aggregate HRR\textsubscript{stab} = 0.302 is reported in
Table~\ref{tab:fullresults}); Qwen3 was evaluated on the same five
perturbation types over a 20-clause sample.}
...
confidence 0.95; Llama~3.2 with confidences spanning 0.05--0.95 but
...
Claude~Sonnet--GPT-4o RIF gap, model-agnostic CAS range, extraction-
sparsity robustness inflation in Llama~3.2).
...
\textbf{Model-agnostic cross-document alignment.}
The tight CAS cluster across all four models (range~$0.315$--$0.326$,
inter-model range~$= 0.011$, variance~$\approx 2.3 \times 10^{-5}$) is
one of the more surprising findings of this study.
Despite an approximately 78$\times$ difference in active parameter
count between the smallest and largest models and substantial
differences in extraction capability, all four models produce nearly
identical cross-document alignment precision under Eq.~\ref{eq:cas}.
This reflects two contributing factors: the embedding model
(\texttt{all-MiniLM-L6-v2}) is held constant across runs, and the
precision component of CAS is bounded below by $\tau=0.50$ for any
pair that clears the threshold.
The practical implication remains significant: even imperfect
extraction can support reliable cross-document alignment when
similarity-based methods are used, lowering the deployment barrier
for UCG construction in resource-constrained RegTech environments.
...
with that in mind.
Llama~3.2 is excluded from the GDPR benchmark due to extraction
...
significant fidelity differences on complex documents; cross-document
alignment is model-agnostic; and composite adversarial robustness
metrics require sub-component decomposition to avoid reporting
extraction-sparsity inflation as genuine robustness.
