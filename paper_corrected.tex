...
independent gold; (ii) among frontier-class extractors,
above-threshold cross-document alignment precision is near-invariant
(CAS range 0.003), making similarity-based alignment robust to
frontier-extractor choice; (iii) the smaller dense open model
achieves the highest OAL (0.820) and CAS (0.360) yet sits at the
bottom of the Basel~III fidelity ordering, revealing \emph{template-
following inflation} --- aggressively uniform outputs that inflate
OAL, CAS, and HRR-Stability without matching gold fidelity,
demonstrating that composite extraction metrics systematically
overstate smaller-model quality when not cross-checked against
gold-aligned RIF. Error analysis of the 30 lowest-fidelity GPT-4o
...
but all four models are statistically tied on conduct-based Reg~BI and
on structurally-clean GDPR under independent human-validated gold;
(ii) among the three frontier-class models, above-threshold
cross-document alignment precision is near-invariant
...
failing to match gold-aligned fidelity on the most discriminating
document---demonstrating that composite extraction metrics
systematically overstate smaller-model quality when not cross-checked
against independent gold-validated RIF.
